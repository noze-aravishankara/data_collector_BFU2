...
class AmmoniaSensor(ABCDevice):
    ...
    def get_instance():
        ...
        """
        if AmmoniaSensor.__instance == None:
            AmmoniaSensor()
        return AmmoniaSensor.__instance

    def __init__(self):
        """Virtually private constructor.
        """
        if AmmoniaSensor.__instance is not None:
            return None
        else:
            AmmoniaSensor.__instance = self
            self._status = Status.UNINITIALIZED
            self._last20 = [.0]*_BUFFER_SIZE

    #@abc.abstractmethod
    def initialize(self, connection_port, baudrate=9600, timeout=10, target_temperature=30):
        ...
        try:
            ...
            get_logger().info("Successfully connected to ammonia sensor at port {}.".format(connection_port))
            return True
    ...
    def _start_collection(self):
        ...
        while(self._status == Status.RUNNING):
            try:
                ...
                self.add_value(str(x.decode('utf-8')).split("\r")[0])
            except:
    ...
    def terminate(self):
        ...
        time.sleep(1)
        get_logger().info("Closing the ammonia sensor connected to {}".format(self._connection.port))
