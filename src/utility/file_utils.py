...
def read_json_file(path_to_file=None):
    """
...
def prepare_output_directory(dir_path):
    ...
    out_dir = os.path.join(cwd,dir_path)
    if not os.path.isdir(out_dir):
        out_dir = os.path.join(os.path.dirname(__file__), "..",
                               "output")  # _get_param_value(parsed_config, "output_directory"))
        # monitor_dir = os.path.join(os.path.dirname(__file__), "..",
        #                            "monitor")
        get_logger().info("out_dir: {}".format(out_dir))
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)
        get_logger().info("out_dir: {}".format(out_dir))
...
def prepare_monitor_directory(dir_path):
    cwd = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    mon_dir = os.path.join(cwd,dir_path)
    # mon_dir = dir_path
    if not os.path.isdir(mon_dir):
        mon_dir = os.path.join(os.path.dirname(__file__), "..",
                               "monitor")  # _get_param_value(parsed_config, "output_directory"))
        get_logger().info("monitor_dir: {}".format(mon_dir))
